...
-- checking if table is having any null values --
SELECT * FROM retail_sales
WHERE 
...
SELECT 
	   category,
	   gender ,
       COUNT(DISTINCT(transaction_id)) AS 'total number of transactions' 
FROM retail_sales
GROUP BY category,
	     gender 
ORDER BY category;
...
```sql
SELECT SUM(total_sale) AS Highest_total_sales, customer_id FROM retail_sales
